...
if [ $RUN_TYPE == "initial" ]; then
    ...
    CELL_TYPES=($(cat "$dir/check_out_$JOB_ID/env_fail.txt"))

elif [ $RUN_TYPE == "run_timed_out" ]; then
    CELL_TYPES=($(cat "$dir/check_out_$JOB_ID/timed_out.txt"))

elif [ $RUN_TYPE == "run_failed" ]; then
    CELL_TYPES=($(cat "$dir/check_out_$JOB_ID/failed.txt"))

elif [ $RUN_TYPE == "run_unaccounted" ]; then
    CELL_TYPES=($(cat "$dir/check_out_$JOB_ID/unaccounted.txt"))
fi
...
/hpc/projects/group.califano/GLM/scGraphLLM/scripts/preprocess_cellxgene.sh "$DIRECTORY" "$cell_type"
