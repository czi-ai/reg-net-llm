...
class LitScGraphLLM(pl.LightningModule):
    ...
    def _step(self, batch, batch_idx):
        ...
        # target_expression_ids = target_expression_ids - NUM_GENES ## shift the rank indices to start from 0
        
        L_mlm_rankonly = self.mlm_loss(predicted_expression_id, target_expression_ids, expression_mask_locs)
        # L_mlm_rankboth = self.mlm_loss(predicted_expression_id, target_expression_ids, both_mask_locs)
        ...
        # self.log(f'{subset}_mlm_rankboth_loss', L_mlm_rankboth, batch_size=(expression_mask_locs|both_mask_locs).sum(), add_dataloader_idx=False )
        ...
        #self.log(f"{subset}_link_pred_loss", L_g, batch_size=(expression_mask_locs|both_mask_locs).sum(), add_dataloader_idx=False)
        return loss
