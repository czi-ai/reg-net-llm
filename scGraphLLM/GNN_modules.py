...
class SPosPE(nn.Module):
    ...
    def forward(self, edge_index, num_nodes):
        ...
        sp_matrix = torch.zeros((num_nodes, num_nodes), device=device)
        ...
        # initialize learnable projection if none
        if self.projection is None:
            self.projection = nn.Linear(sp_embeddings.size(1), self.projection_dim).to(sp_embeddings.device)
