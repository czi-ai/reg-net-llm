...
def _aracne_to_edge_list(network, genes):
    edges = network[['regulator.values', 'target.values']]
    ...
    edge_list = torch.tensor(edges)
    edge_weights = torch.tensor(edges['mi.values'])
...
def node_batchin(adata, network, genes, batch_size=64, 
    ...
    dataloader = NeighborLoader(data=graph_dataset, replace=False, num_neighbors=[neigborhood_size] * num_hops, 
                                ...
                                weight_attr=True, batch_size=batch_size
                                )
